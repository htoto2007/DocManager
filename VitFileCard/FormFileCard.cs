...
namespace VitFileCard
{
    public partial class FormFileCard : Form
    {
        ...
        public CardPropCollection[] GetProps()
        {
            CardPropCollection[] cardPropCollections = new CardPropCollection[panelCardProps.Controls.Count / 2];
            int iterator = 0;
            foreach (Control control in panelCardProps.Controls)
            {
                if (control.Name.Split('_')[0] == "tb")
                {
                    cardPropCollections[iterator] = getValueByControl(control);
                    iterator++;
                }
            }
            return cardPropCollections;
        }
        ...
        private void comboBoxTypeCard_SelectedIndexChanged(object sender, EventArgs e)
        {
            ComboBox comboBox = (ComboBox)sender;
            ClassTypeCard classTypeCard = new ClassTypeCard();
            var idTypeCard = classTypeCard.getIdByName(comboBox.Text);
            ClassCardProps classTypeCardProps = new ClassCardProps();
            ClassCardProps.CardPropsCollection[] typeCardProps = classTypeCardProps.getInfoByIdTypeCard(idTypeCard);
